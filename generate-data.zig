...
fn getDateTime(timestamp: i64) ![]const u8 {
    ...
    assert(hour>=0 and hour<25);
    assert(min>=0 and min<61);
    assert(sec>=0 and sec<61);
    return try std.fmt.bufPrint(&Static.buf,
        "{d:0>4}-{d:0>2}-{d:0>2}T{d:0>2}:{d:0>2}:{d:0>2}Z",
        .{year, month, month_day, hour, min, sec});
}
